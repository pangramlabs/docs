...
Extended analysis with adaptive boundaries. **Deprecated** — use `predict()` instead.
...
Classify a long document using a sliding window. **Deprecated** — use `predict()` instead.
