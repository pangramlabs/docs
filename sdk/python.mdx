...
Upload a single `.docx`, `.pdf`, or `.rtf` document for AI detection. Pangram extracts the document text and returns the same prediction fields as `predict()`, plus the uploaded `filename`.
...
<ParamField body="file_path" type="string" required>
  Path to the file to upload and analyze.
</ParamField>
...
<ParamField body="timeout" type="float" default="300">
  Maximum number of seconds to wait for the upload request to complete.
</ParamField>

**Returns**

A dictionary with the extracted text, prediction fields, window-level analysis, and uploaded filename.
...
All other prediction fields match `predict()`, including `headline`, `prediction`, `prediction_short`, `fraction_ai`, `fraction_ai_assisted`, `fraction_human`, and `windows`.

---

### predict_files()

Upload multiple `.docx`, `.pdf`, or `.rtf` documents in one request.
...
<ParamField body="file_paths" type="list" required>
  Paths to files to upload and analyze.
</ParamField>

<ParamField body="public_dashboard_link" type="boolean" default="false">
  Whether to include a public dashboard link in each completed response.
</ParamField>
...
A list of per-file result dictionaries. Each result has the same shape returned by `predict_file()`.
