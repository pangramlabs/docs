...
<ResponseField name="completed_at" type="string">
  Job completion timestamp. `None` while the job is not terminal.
</ResponseField>
...
```python
results = client.get_bulk_results(bulk_id, offset=0, limit=100)
...
A dictionary containing `bulk_id`, `offset`, `limit`, `total_items`, `items`, and `failed_items`. Successful completed items include `result` with the same shape returned by `predict()`. In-progress items have `result` set to `None`.
